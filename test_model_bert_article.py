...
class TestModel:
    ...
    def __get_most_similar_token_levenshtein(self, target_word):
        ...
        list_of_similars = list_of_similars[0:indUntil2]


        return list_of_similars

    def __get_most_similar_token_mix(
        self, sentence, target_word, N=500
    ):
...
if __name__ == "__main__":
    ...
    if model_path and output_file_name:
        ...
        tm = TestModel(main_path=main_path, model_path=model_path, version=version)
    elif output_file_name:
        tm = TestModel(main_path=main_path, output_file_name=output_file_name, version=version)
    else:
        tm = TestModel(main_path=main_path, version=version)
