...
class Preprocess:
    def __init__(self, main_path, datasets, epochs=5):
        ...
        self.KEYBOARD_ERRORS_DIR = main_path + '/dictionary/keyboard_realword_errors.txt'
        self.SUBSTITUTION_ERRORS_DIR = main_path + '/dictionary/substitution_realword_errors.txt'
        self.HOMOPHONE_ERRORS_DIR = main_path + '/dictionary/homophone_realword_errors.txt'
        ...
        self.TRAIN_FINAL_DATASET = main_path + '/train test datasets/train/final_dataset.txt'
        self.CORRECT_SENTNCES = main_path + '/train test datasets/train/correct_sentences.txt'
        self.METADATA = main_path + '/train test datasets/train/metadata.txt'
        self.CORRUPTED_SENTENCES = main_path + '/train test datasets/train/corrupted_sentences.txt'
        self.MASKED_SENTENCES = main_path + '/train test datasets/train/masked_sentences.txt'
    ...
    def __generate_realword_errors_per_token_files(self):
        with open(self.KEYBOARD_ERRORS_DIR, 'w', encoding='utf-8') as f1, \
            open(self.SUBSTITUTION_ERRORS_DIR, 'w', encoding='utf-8') as f2:
            
            for i, word in enumerate(self.dictionary):    
                keyboard_realword_errors = self.__generate_keyboard_errors_per_token(word.strip())
                substitution_realword_errors = self.__generate_substitution_errors_per_token(word.strip())
    ...
    def __prune_dataset(self):
        with open(self.RAW_DATASET_DIR, 'r', encoding='utf-8') as f1, \
            open(self.PRUNE_DATA, 'w', encoding='utf-8') as f2:
    ...
    def __generate_keyboard_nonrealword_errors_per_line(self, li):
        ...
        for t in tokens:
            if len(t) > 2:
                ...
                while newToken in self.dictionary or t[indexOfMisspelled] == self.letters[indexOfAlternative]:
                    indexOfMisspelled = random.randrange(0, tokenLength)
                    indexOfAlternative = random.randrange(0, self.NUMOFLETTERS)
                    newToken = self.__manual_replace(t, self.letters[indexOfAlternative], indexOfMisspelled)
                token = t
    ...
    def __generate_final_dataset_test(self):
        with open(self.PRUNE_DATA, 'r', encoding='utf-8') as f1, \
            open(self.TEST_DATASET, 'w', encoding='utf-8') as f2:
            c0 = 0
            c1 = 0
            c2 = 0
            c3 = 0
            ...
            for line in f1:
                ...
                if can1 is not False:
                    newLine, oldToken, newToken = self.__generate_homophone_realword_errors_per_line(line, can1)
                    ...
                    if i == 2:
                        newLine, oldToken, newToken = self.__generate_keyboard_nonrealword_errors_per_line(line)
                        typeOfError = 'keyboard_nonrealword_error'
                        c2 += 1
                    elif i == 3:
                        newLine, oldToken, newToken = self.__generate_polymorph_nonrealword_per_line(line)
                        typeOfError = 'polymorph_nonrealword_error'
                        c3 += 1
                    elif i == 4:
                        newLine, oldToken, newToken = self.__generate_substitution_nonrealword_errors_per_line(line)
                        typeOfError = 'substitution_nonrealword_error'
                        c4 += 1
                    elif i == 5 and can5 is not False:
                        newLine, oldToken, newToken = self.__generate_substitution_realword_errors_per_line(line, can5)
                        typeOfError = 'substitution_realword_error'
                        c5 += 1
                    elif i == 6 and can6 is not False:
                        newLine, oldToken, newToken = self.__generate_keyboard_realword_errors_per_line(line, can6)
                ...
                f2.write(newLine.strip() + "^" + str(typeOfError) + "^" + str(oldToken) + "^" + str(newToken) + "\n")
    ...
    def __generate_final_dataset_train(self, id_=1):
        with open(self.PRUNE_DATA, 'r', encoding='utf-8') as f1, \
            open(self.TRAIN_DATASET + f'final_dataset_{id_}.txt', 'w', encoding='utf-8') as f2:
            c0 = 0
            c1 = 0
            c2 = 0
            c3 = 0
    ...
    def __generate_meta_datasets(self):
        with open(self.TRAIN_FINAL_DATASET, 'r', encoding='utf-8') as f, \
            open(self.CORRECT_SENTNCES, 'w+', encoding='utf-8') as f1, \
            open(self.METADATA, 'w+', encoding='utf-8') as f2, \
            open(self.CORRUPTED_SENTENCES, 'w+', encoding='utf-8') as f3, \
            open(self.MASKED_SENTENCES, 'w+', encoding='utf-8') as f4:
